...
// Google Gemini OpenAI 호환 모드 URL
...
export async function proxyGoogle(
  ...
  const targetUrl = `${GOOGLE_BASE_URL}/${endpoint}`;
  ...
  if (usage && tokenResult.orgId && finalModel) {
    const cost = await estimateCost('google', finalModel, usage.inputTokens, usage.outputTokens);
